...
function UserDashboard() {
  ...
  const handleReviewSubmit = async (reviewData) => {
    await postData(POST_REVIEW_PATH, reviewData);
  };
  ...
  useEffect(() => {
    const queryParams = new URLSearchParams({
      id: getCookie("userId"),
      isServiceman:
        JSON.parse(localStorage.getItem("response"))?.isServiceman === "P",
    }).toString();
    const url = `${BOOKING_BASE_PATH}?${queryParams}`;
    const reviewUrl = `${GET_REVIEW_PATH}?${queryParams}`;

    Promise.all([getData(url), getData(reviewUrl)])
      .then((responses) => {
                  ...
                  {order.map((row, index) => {
                    return (
                      <tr key={index}>
                        ...
                        <td>
                          {row.serviceName} <br /> {row.description}
                        </td>
                        <td>
                          Service Date :{" "}
                          {dateFormatterWithDayName(row.dateOfAppointment)}{" "}
                          <br /> Booking Date :{" "}
                          {dateFormatterWithDayName(row.dateOfBooking)}{" "}
                        </td>
                        <td>{row.address}</td>
                        ...
                        {row.status === "Accepted" ? (
                          <Link
                            ...
                            to={`/map/${getCookie("userId")}`} // Provide the appropriate route if using React Router
                          >
                            MAP
                          </Link>
                        ) : (
...
const ReviewCard = ({ review }) => {
  ...
  return (
    <div
      className={`${style["review-container"]} bg-gray-700 text-white rounded-lg shadow-lg overflow-hidden mb-4`}
    >
